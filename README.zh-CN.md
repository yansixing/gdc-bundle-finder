...
Sonniss 每年 GDC 期间免费放出的 **#GameAudioGDC** 音效包（2015 到 2026 年，共 78 卷 zip，约 200 GB），这里把每一卷里的每个音频文件都建了索引。搜一下，就知道你要的声音在哪一年的第几卷，只下那一卷就行，不用整套几十 GB 全拉下来。
...
| `zip_listings.json` | 78 卷 zip 的原始目录数据（索引的源数据）。 |
...
分卷大致按"供应商 - 库名"文件夹的字母顺序切分，边界处有少量交叉。2021、2022、2023 三年合并发布为一个 14 卷的包。2025 年没有发包；2026 年的包单独放在新站上，Sonniss 说后续可能还会追加，所以索引是抓取时的快照。
